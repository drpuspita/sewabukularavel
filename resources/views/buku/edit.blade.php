    ...
    <form method="POST" action="{{ route('buku.update', $buku -> id) }}">
        @csrf
            ...
            <div class="form-group">
                ...
                <input type="number" name="jumlah_halaman" min="0" value="{{ $buku->jumlah_halaman }}" 
                    class="form-control">
            </div>
            <div class="form-group">
                <label>ISBN</label>
            ...
            <div class="form-group">
                <?php
                    ...
                    foreach(range(date('Y', strtotime(date('Y', time()) . ' + 625 day')), 
                        $tahun_awal) as $tahun_terbit){
                        print '<option value="'.$tahun_terbit.'"'.
                            ($tahun_terbit === $already_selected_value ? ' selected="selected"' : '').
                            '>'.$tahun_terbit.'</option>';
                    }
                    print '</select>' ;
                ?>
