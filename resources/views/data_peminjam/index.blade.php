            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <img src="{{ asset('foto_peminjam/foto_kosong.jpeg') }}" alt="" style="width:50px;height:60px;">
                        @else
                        <img src="{{ asset('foto_peminjam/'.$peminjam -> foto) }}" alt="" style="width:50px; height:60px;">
                        @endif
                    </td>
                    <td>
                        <a href="{{ route('data_peminjam.edit', $peminjam -> id) }}" class="btn btn-warning btn-sm">Edit</a>
                    </td>
                    <td>
                        <form action="{{ route('data_peminjam.destroy', $peminjam -> id) }}" method="POST">
                            @csrf
                            <button class="btn btn-warning btn-sm" onclick="return confirm('Anda yakin ingi menghapus data ini?')">Hapus</button>
                        </form>
                    </td>
                </tr>
                @endforeach
